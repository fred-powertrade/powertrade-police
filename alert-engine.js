...
async function sendSlack(alerts, summary) {
  ...
  const blocks = [
    { type:'header', text:{ type:'plain_text', text:`🚨 PT Police — ${alerts.length} alert${alerts.length!==1?'s':''}`} },
...
async function sendTelegram(alerts, summary) {
  ...
  let text = `🚨 <b>PT Police — ${alerts.length} alert${alerts.length!==1?'s':''}</b>\n${summary}\n\n`;
...
// ── Main ────────────────────────────────────────────────────────
async function main() {
  console.log(`\n═══ PT Police Alert Engine — ${new Date().toISOString()} ═══\n`);
